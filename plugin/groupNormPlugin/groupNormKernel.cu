...
template <int32_t tTHREADS_PER_BLOCK>
__global__ void groupNormNHWCScaleKernel(GroupNormNHWCParams params, float epsilon)
{
    // The instance in the batch.
    int32_t ni = blockIdx.z;
    // The channel loaded by that thread (2 channels per thread for F16x2).
    ...
    float var = sumSq * params.invHWC - (mean * mean) + epsilon;
    if (var <= 0.F) {
        printf("TTTTTTTTT %f %f %f %f\n", var, sumSq, mean, epsilon);
    }
...
void groupNormNHWCScale(GroupNormNHWCParams const& params, float epsilon, cudaStream_t stream)
{
    ...
    switch (params.cPerBlock)
    {
    case 320: groupNormNHWCScaleKernel<160><<<grid, 160, 0, stream>>>(params, epsilon); break;
    case 480: groupNormNHWCScaleKernel<256><<<grid, 256, 0, stream>>>(params, epsilon); break;
    case 256: groupNormNHWCScaleKernel<128><<<grid, 128, 0, stream>>>(params, epsilon); break;
    case 128: groupNormNHWCScaleKernel<64><<<grid, 64, 0, stream>>>(params, epsilon); break;
    default: PLUGIN_FAIL("Not implemented");
    }
