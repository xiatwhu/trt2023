...
size_t constexpr kSERIALIZATION_SIZE{sizeof(float) + sizeof(int32_t) + sizeof(int32_t)};
} // namespace
...
GroupNormPlugin::GroupNormPlugin(std::string const& name, float epsilon, int32_t bSwish, int32_t group)
    : mName(name)
    , mEpsilon(epsilon)
    , mBSwish(bSwish)
    , mGroup(group)
    ...
    void const* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    try
    {
        ...
        groupNormNHWCScale(mParams, mEpsilon, stream);
...
IPluginV2* GroupNormPluginCreator::createPlugin(char const* name, PluginFieldCollection const* fc) noexcept
{
    try
    {
        ...
        for (int32_t i = 0; i < fc->nbFields; ++i)
        {
            ...
            if (fc->fields[i].name == std::string("group"))
            {
                group = static_cast<int32_t>(*(static_cast<int32_t const*>((fc->fields[i].data))));
                continue;
            }
        }
        return new GroupNormPlugin(name, epsilon, bSwish, group);
    }
    catch (std::exception const& e)
    {
        caughtError(e);
